...
* You can create an interface by simply defining a class, then another class can **implement** it.
* When you `implement` an interface, you must provide **all method and property implementations**.

### 2.2 Multiple Interface Inheritance

* Dart allows a class to `implement` **more than one interface**.
* You separate multiple interfaces with a comma (`class MyClass implements Interface1, Interface2`).
* This approach lets you combine behaviors from multiple sources without class inheritance conflicts.
...
* Interfaces in Dart are **implicit** — every class can be used as an interface.
* The `implements` keyword forces **full reimplementation** of all methods and properties.
* Multiple interface inheritance is a way to achieve **polymorphism** without multiple class inheritance.
* Unlike `extends` (inheritance), `implements` **does not** reuse code from the interface class — you must write your own implementations.
...
* Keep each interface **small and focused** on one responsibility (Interface Segregation Principle).
* Use descriptive names for interfaces (`Logger`, `Serializable`, `Auth`) so their purpose is clear.
* Avoid putting implementation logic in interface-defining classes.
